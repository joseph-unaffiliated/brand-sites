...
const ogImagePath = process.env.NEXT_PUBLIC_SITE_OG_IMAGE || "/hip-photo.png";
const faviconIco = process.env.NEXT_PUBLIC_SITE_FAVICON || "/hip-favicon.ico";
const faviconPng = process.env.NEXT_PUBLIC_SITE_FAVICON_PNG || "/hip-favicon.png";
...
export default async function RootLayout({ children }) {
  ...
  try {
    ...
    latestIssueImage = entries[0]?.mainImage ?? null;
  } catch {
  ...
  try {
    ...
    const wordSlug = requestHeaders.get("x-word-slug");
    if (wordSlug) {
      const entry = await getSlangEntryBySlug(wordSlug);
      initialPageFillImage = entry?.mainImage ?? null;
    }
  } catch {
    /* Word nav fill optional in dev */
  }

...
/**
 * @param {{ recommendedArticles?: Array<{ _id?: string, slug: string, title: string, summary?: string, mainImage: string }> }} props
 */
export default function SubscribedContent({ recommendedArticles = [] }) {
  ...
  useEffect(() => {
    ...
    // refresh is intentionally omitted — including it re-ran this effect in a loop
  }, [searchParams]);
  ...
  return (
    <div className={styles.wrap} data-subscription-landing>
      <div className={`${styles.card} ${actions.cardWide}`}>
        ...
        {showRecs && (
          <>
            <SubscriptionSuccessRecs email={recEmail} />
            <SubscriptionSuccessArticleRecs articles={recommendedArticles} />
          </>
        )}
      </div>
    </div>

...
export default async function NotFound() {
  ...
  const suggested = pickRandomArticles(allRecipes, { count: SUGGESTED_COUNT });
      ...
      <section className="articlebody-section">
        ...
        {suggested.length > 0 && (
          <div className={articleStyles.readMoreOuter}>
            <section className={articleStyles.readMore} aria-label="Suggested recipes">
              <div className={articleStyles.readMoreGrid}>
                {suggested.map((rec) => (
                  ...
                  >
                    ...
                    {rec.description && <p className={articleStyles.readMoreDek}>{rec.description}</p>}
                  </Link>
                ))}
              </div>
            </section>

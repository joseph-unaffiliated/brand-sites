 ...
 * NEXT_PUBLIC_RETENTION_SITE_ID on Vercel for this brand).
 *
...
const ogImagePath = process.env.NEXT_PUBLIC_SITE_OG_IMAGE || "/tec-photo.png";
const faviconIco = process.env.NEXT_PUBLIC_SITE_FAVICON || "/tec-favicon.ico";
const faviconPng = process.env.NEXT_PUBLIC_SITE_FAVICON_PNG || "/tec-favicon.png";
...
export default async function RootLayout({ children }) {
  ...
  try {
    ...
    latestIssueImage = recipes[0]?.mainImage ?? null;
  } catch {
  ...
  try {
    ...
    const recipeSlug = requestHeaders.get("x-recipe-slug");
    if (recipeSlug) {
      const recipe = await getRecipeBySlug(recipeSlug);
      initialPageFillImage = recipe?.mainImage ?? null;
    }
  } catch {
    /* Recipe nav fill optional in dev */
  }

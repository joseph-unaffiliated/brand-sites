...
/**
 * Same path for both states — the filled ♥ shape the user prefers when
 * selected. Unsaved = stroke outline; saved = solid fill.
 */
...
/**
 * Heart toggle for saving a recipe. Requires subscription — otherwise opens
 * the subscribe banner. `variant="card"` is icon-only for recipe cards.
 */
export default function FavoriteButton({ slug, variant = "full" }) {
  ...
  const handleClick = (e) => {
    ...
    e.stopPropagation();
    if (!isSubscribed) {
      promptSubscribeToFavorite(slug);
      return;
    }
  ...
  return (
    ...
    >
      ...
      {variant === "full" ? (
        <span className={styles.label}>{showFilled ? "Saved" : "Save recipe"}</span>
      ) : null}
    </button>
  );
}

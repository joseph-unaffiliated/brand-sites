...
2. **Root Directory:** `apps/hookuplists`, `apps/thepicklereport`, `apps/thekissandtell`, `apps/the90sparent`, `apps/theeyeballerscookbook`, `apps/hipspeak`, etc. (one Vercel project per app).
...
- Article listing and `/article/[slug]` load from Sanity (for theeyeballerscookbook: `/recipes` and `/recipe/[slug]`; for hipspeak: `/archive` and `/word/[slug]`).
